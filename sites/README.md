...
### For Known Systems (Niagara, Meatball Tracers, etc.)
...
3. **Edit with your details**
   - Replace `your_niagara_host` → your actual hostname
   - Replace `your_site_name` → a descriptive name
...
| -------- | -------- | -------- | -------- | -------- |
| **Tridium** | `niagara.template.json` | 2-step multi-page | ✅ | Most common BAS. Username on /prelogin, password on /login |
| **Meatball** | `trane-tracer-synchrony.template.json` | Single form | ✅ | Uses /hui/index.html, hash routing |
| **React/SPA** | `react-url-based.template.json` | Single form | ✅ | Hash-routed navigation (most reliable) |
| **React/SPA** | `react-click-based.template.json` | Single form | ✅ | Click-based nav (less reliable) |
| **Johnson Controls** | `johnson-controls.template.json` (planned) | Unknown | ❌ | Template planned - needs bootstrap recording |
| **Custom/Generic** | `example.json` | Flexible | ✅ | Start here if not sure |
...
⚠️ **Never commit credentials to git** - Add `sites/*.json` to `.gitignore` if they contain real credentials.
